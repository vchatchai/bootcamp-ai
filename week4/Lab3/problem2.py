...
Output = np.concatenate(
    [np.matrix(np.zeros([100, 1])), np.matrix(np.ones([100, 1]))])
...
# wrute the hypothesis function based on TF methods here
hypothesis_function = tf.div(1.0, tf.add(
    1.0, tf.exp(tf.multiply(-1.0, tf.add(z, theta0)))))

'''
Step 3: Define the cost function
'''

log = tf.log(hypothesis_function)

log2 = tf.log(tf.subtract(1.0, hypothesis_function))

y = tf.subtract(1.0, Y)
m = tf.multiply(y, log2)
tf.reduce_mean(log, m)

cost_function = tf.multiply(-1.0, tf.reduce_mean(log, tf.multiply(y, log2)))
'''
Step 4: Use gradient descent with learning rate of 0.03 to minimize the cost function
'''

optimizer = tf.train.GradientDescentOptimizer(
    learning_rate=0.03).minimize(cost_function)
...
with tf.Session() as session:
    ...
    for i in range(30000):
        # run the optimizer and cost function in a session w.r.t. matrices Input and Output})
        _, cost = session.run([optimizer, cost_function],
                              feed_dict={X: Input, Y: Output})

        print("Epoch: {0}, cost = {1}".format(i + 1, cost))

    print("theta0: {0}, theta1: {1}, theta2: {2}".format(session.run(
        theta0[0]), session.run(thetas[0][0]), session.run(thetas[1][0])))
