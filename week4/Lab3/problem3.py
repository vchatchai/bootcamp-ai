...
y = np.concatenate([np.matrix(np.zeros([100, 1])),
                    np.matrix(np.ones([100, 1]))])
...
# You may also rename this variable to 'm' to keep it consistent with our slide.
...
while not converged:
    ...
    for i in range(number_of_samples):
        ...
        # Here, x.shape = (3, ) i.e. a tuple.
        x = X_Augmented_By_One[individual, :]
    ...
    print('theta0: {0}, theta1: {1}, theta2: {2}'.format(
        theta[0], theta[1], theta[2]))

    predicted_y = hypothesis(X_Augmented_By_One, theta)
    # We predict y = 1 if h(theta^T x) >= 0.5
    predicted_y_crisp = predicted_y >= 0.5
    ...
    log_likelihood = np.add(np.log(predicted_y),( np.log(1 - predicted_y)))
