...
cost_function = tf.multiply(tf.divide(1, 2), tf.reduce_mean(
    tf.pow(Y - hypothesis_function, 2)))
...
with tf.Session() as session:
    ...
    writer = tf.summary.FileWriter('./graphs/liner_regression', session.graph)

    for i in range(30000):
        session.run(optimizer, feed_dict={X: data.T[0], Y: data.T[1]})
        summary, cost = session.run([merged, cost_function], feed_dict={
                                    X:  data.T[0], Y: data.T[1]})
        writer.add_summary(summary, i)
        print("Epoch:{0},cost = {1}, theta0 = {2}, theta1={3}".format(
            i+1, cost, session.run(theta0), session.run(theta1)))

    print("Optimization Finished")
    training_cost = session.run(cost_function, feed_dict={
                                X: data.T[0], Y: data.T[1]})
    print("Cost = ", training_cost, "theta0 =", session.run(
        theta0), "theta1 = ", session.run(theta1), '\n')
